...
| 触发器 | 何时 fire | 谁消费 |
| --- | --- | --- |
| `OnChildClosed`（内部 hook） | 某子 issue merged / closed 时由 issue 模块回调 | 引擎（重算父的前沿） |
| `sub_issue.merged` / `sub_issue.closed` | 某子 issue merged / closed 时，向**父 issue** 投递（cause = 该子 issue） | 任意 agent role（可订阅的公开触发器） |
...
- **触发器**：`triggers.go` + `sub_issue.merged`、`sub_issue.closed`、`issue.ready`（及可选 `plan.review`）；同步更新解析器 / 派发器 / 审计 UI 的 enum 分支。引擎内部通过 `OnChildClosed` hook 接收通知（不依赖公开触发器）。
...
- **merge/close 路径挂钩**：在 issue 模块的 merge / close 路径中（见 `handler.go` `fireSubIssueEvent`）：若 issue 有父，引擎通过 `planEngine.OnChildClosed` 回调获知，同时通过 spawner 向父 issue 投递 `sub_issue.merged` / `sub_issue.closed` 供 agent 订阅。
