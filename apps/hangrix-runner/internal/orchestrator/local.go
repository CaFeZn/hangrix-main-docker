...
// EnsureImagePulled is a no-op — local mode has no Docker. Always returns
// skipped=true (the agent binary is already on disk).
func (o *LocalOrchestrator) EnsureImagePulled(_ context.Context, image string, _ chan<- PhaseEvent) (bool, error) {
	// Local mode: nothing to pull.
	return true, nil
}

// BuildImage is a no-op — local mode has no Docker.
func (o *LocalOrchestrator) BuildImage(_ context.Context, image string, _ *BuildSpec, _ string, _ chan<- PhaseEvent) (bool, error) {
	return true, nil
}

// CreateAndStartWorkflowContainer returns a synthetic container ID.
func (o *LocalOrchestrator) CreateAndStartWorkflowContainer(_ context.Context, image string, _ []string, _ string, _ []Volume, _ chan<- PhaseEvent) (string, error) {
	if image == "" {
		return "", fmt.Errorf("orchestrator: image is required")
	}
	id := atomic.AddInt64(&o.nextLocalID, 1)
