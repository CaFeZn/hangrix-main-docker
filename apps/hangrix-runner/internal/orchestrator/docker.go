...
func (o *DockerOrchestrator) createContainer(ctx context.Context, t Task) (string, error) {
	...
	for _, vol := range t.Volumes {
		args = append(args, "-v", o.volumeArg(vol))
	}
...
//
// This is now a backward-compatible wrapper; new callers should use
// EnsureImagePulled / BuildImage / CreateAndStartWorkflowContainer
// directly to observe phase-level progress.
func (o *DockerOrchestrator) WorkflowContainer(ctx context.Context, image string, build *BuildSpec, entrypoint []string, hostWorkdir string, env map[string]string, volumes []Volume) (string, error) {
	...
	// Resolve the image via the new phase-aware methods (no events channel
	// — backward-compatible silence).
	if build != nil {
		if _, err := o.BuildImage(ctx, image, build, hostWorkdir, nil); err != nil {
			return "", fmt.Errorf("build image: %w", err)
		}
	} else {
		if _, err := o.EnsureImagePulled(ctx, image, nil); err != nil {
			return "", fmt.Errorf("pull image: %w", err)
		}
	}

	cid, err := o.CreateAndStartWorkflowContainer(ctx, image, entrypoint, hostWorkdir, volumes, nil)
	if err != nil {
		return "", fmt.Errorf("create/start container: %w", err)
	}
	return cid, nil
}

// dockerImageExists probes whether an image tag is cached locally.
func (o *DockerOrchestrator) dockerImageExists(ctx context.Context, image string) bool {
	probe := exec.CommandContext(ctx, o.bin, "image", "inspect", "--format={{.Id}}", image)
	probe.Stdout = io.Discard
	probe.Stderr = io.Discard
...
// Returns (skipped=true, nil) on cache hit; (false, nil) on successful pull.
// When events is non-nil, each output line is forwarded as a PhaseEvent.
func (o *DockerOrchestrator) EnsureImagePulled(ctx context.Context, image string, events chan<- PhaseEvent) (bool, error) {
	if image == "" {
		return false, fmt.Errorf("image is required")
	}
	if o.dockerImageExists(ctx, image) {
		return true, nil
	}
	cmd := exec.CommandContext(ctx, o.bin, "pull", image)
	return false, o.streamDockerCmd(ctx, cmd, events)
}

// BuildImage builds the image from the supplied BuildSpec when not present
// locally. Returns (skipped=true, nil) on cache hit. When events is non-nil,
// each output line is forwarded as a PhaseEvent.
func (o *DockerOrchestrator) BuildImage(ctx context.Context, image string, build *BuildSpec, hostWorkdir string, events chan<- PhaseEvent) (bool, error) {
	if image == "" {
		return false, fmt.Errorf("image is required")
	}
	if build == nil {
		return false, fmt.Errorf("build spec is required")
	}
	if build.Dockerfile == "" {
		return false, fmt.Errorf("build.dockerfile is required")
	}
	if o.dockerImageExists(ctx, image) {
		return true, nil
	}

	dockerfilePath := filepath.Join(hostWorkdir, build.Dockerfile)
	if _, err := os.Stat(dockerfilePath); err != nil {
		return false, fmt.Errorf("dockerfile not found at %s: %w", dockerfilePath, err)
	}
	contextDir := hostWorkdir
	if build.Context != "" && build.Context != "." {
		contextDir = filepath.Join(hostWorkdir, build.Context)
	}
	if _, err := os.Stat(contextDir); err != nil {
		return false, fmt.Errorf("build context not found at %s: %w", contextDir, err)
	}
...
// CreateAndStartWorkflowContainer creates + starts a workflow container
// without image resolution. When events is non-nil, diagnostics are forwarded.
func (o *DockerOrchestrator) CreateAndStartWorkflowContainer(ctx context.Context, image string, entrypoint []string, hostWorkdir string, volumes []Volume, events chan<- PhaseEvent) (string, error) {
	if image == "" {
		return "", fmt.Errorf("image is required")
	}
	...
	for _, vol := range volumes {
		createArgs = append(createArgs, "-v", o.volumeArg(vol))
	}
	createArgs = append(createArgs, image)
	createArgs = append(createArgs, cmdArgs...)

	var stdout, stderr bytes.Buffer
	cmd := exec.CommandContext(ctx, o.bin, createArgs...)
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		// Forward stderr as events when available.
		if events != nil {
			for _, line := range strings.Split(strings.TrimSpace(stderr.String()), "\n") {
				if line != "" {
					events <- PhaseEvent{Stream: "stderr", Line: line}
				}
			}
		}
	...
	// docker start
	startCmd := exec.CommandContext(ctx, o.bin, "start", id)
	...
	if err := startCmd.Run(); err != nil {
		_ = o.run(context.Background(), "rm", "-f", id)
		if events != nil {
			for _, line := range strings.Split(strings.TrimSpace(startStderr.String()), "\n") {
				if line != "" {
					events <- PhaseEvent{Stream: "stderr", Line: line}
				}
			}
		}
		return "", fmt.Errorf("docker start %s: %w (stderr: %s)", id, err, strings.TrimSpace(startStderr.String()))
	}
	return id, nil
}

// streamDockerCmd runs a docker command and forwards its merged output
// (stdout + stderr) line-by-line to the events channel. When events is nil
// the output is discarded. Returns the underlying exec error on failure.
//
// Uses StdoutPipe + StderrPipe + cmd.Start() so lines are forwarded in
// real time — no buffering until cmd.Run() returns. Two goroutines read
// via bufio.Scanner; the caller drains events concurrently (the channel
// is sized to avoid head-of-line blocking).
func (o *DockerOrchestrator) streamDockerCmd(ctx context.Context, cmd *exec.Cmd, events chan<- PhaseEvent) error {
	if events == nil {
		cmd.Stdout = io.Discard
		cmd.Stderr = io.Discard
		return cmd.Run()
	}
	// docker pull/build output goes to stderr; we also capture stdout
	// for tools that write progress there.
	stdout, err := cmd.StdoutPipe()
	if err != nil {
	...
	// Read both pipes concurrently; forward each line as a PhaseEvent.
	type drainResult struct {
		stream string
		err    error
	}
	done := make(chan drainResult, 2)

	drain := func(stream string, r io.Reader) {
		scanner := bufio.NewScanner(r)
		for scanner.Scan() {
			events <- PhaseEvent{Stream: stream, Line: scanner.Text()}
		}
		done <- drainResult{stream: stream, err: scanner.Err()}
	}

	go drain("stdout", stdout)
	go drain("stderr", stderr)

	// Wait for both scanners, then collect the command exit status.
	for i := 0; i < 2; i++ {
		<-done
	}
...
// volumeArg renders one Volume into a docker `-v` value. Named volumes
// (HostPath=="") render as `<name>:<mount>`; bind-mount volumes
// (HostPath!="") render as `<absMappedHost>:<mount>:ro` — read-only by
// design since the only reserved bind today is the agent binary directory,
// which the agent has no business writing back to. Same helper used by
// both the session createContainer path and the workflow
// CreateAndStartWorkflowContainer path so the mount semantics stay aligned.
func (o *DockerOrchestrator) volumeArg(vol Volume) string {
	if vol.HostPath != "" {
		return o.absMount(vol.HostPath, vol.Mount, true)
	}
	return vol.Name + ":" + vol.Mount
}
