...
// EnsureImagePulled is a no-op stub — workflows use the real Docker
// orchestrator in tests that need phase events.
func (f *FakeOrchestrator) EnsureImagePulled(_ context.Context, image string, events chan<- PhaseEvent) (bool, error) {
	if image == "" {
		return false, fmt.Errorf("orchestrator: image is required")
	}
	// Simulate cache hit so tests don't actually pull.
	return true, nil
}

// BuildImage is a no-op stub.
func (f *FakeOrchestrator) BuildImage(_ context.Context, image string, _ *BuildSpec, _ string, events chan<- PhaseEvent) (bool, error) {
	if image == "" {
		return false, fmt.Errorf("orchestrator: image is required")
	}
	return true, nil
...
// CreateAndStartWorkflowContainer returns a synthetic container ID.
func (f *FakeOrchestrator) CreateAndStartWorkflowContainer(_ context.Context, image string, _ []string, _ string, _ []Volume, events chan<- PhaseEvent) (string, error) {
	if image == "" {
		return "", fmt.Errorf("orchestrator: image is required")
	}
	return fmt.Sprintf("fake-wf-container-%d", len(f.removed)), nil
