...
func Serve(ctx context.Context, cfg *config.Config) error {
	...
	// In --mock mode, swap the orchestrator wholesale. The old code-path
	// kept a separate SessionOrchestrator for the session driver only,
	// but that driver is gone now (agents run through the same workflow
	// path as user jobs), so the wholesale swap matches the new model.
