...
test.describe('workflows', () => {
  ...
  test('agent tab is present and shows agent workflow runs', async ({ page }) => {
    await ensureLoggedIn(page, owner, PASSWORD)
    await page.goto(`/${owner}/${repoName}/workflows`)
    await page.waitForSelector('[data-slot="tabs-trigger"]', { timeout: 15_000 })
    ...
    await expect(agentTab).toBeVisible({ timeout: 5_000 })

    // Click the Agent tab.
    await agentTab.click()
    await page.waitForTimeout(500)

    // URL should contain kind=agent.
    await expect(page).toHaveURL(/kind=agent/, { timeout: 5_000 })

    // The page should show either agent runs or the agent-specific empty state.
    const agentEmpty = page.getByText(/Agent.*工作流|No agent workflow|暂无 Agent/i)
    const listItems = page.locator('ul > li')
    await expect(agentEmpty.or(listItems.first()).first()).toBeVisible({ timeout: 10_000 })

    // The dispatch button should be hidden on the agent tab.
    const dispatchBtn = page.getByRole('button', { name: /Dispatch|手动触发|触发/i })
    await expect(dispatchBtn).not.toBeVisible()

    // Switch back to the All tab — dispatch button should reappear.
    const allTab = page.getByRole('tab', { name: /All|全部/i })
    if (await allTab.isVisible().catch(() => false)) {
      await allTab.click()
      await page.waitForTimeout(300)
      await expect(page.getByRole('button', { name: /Dispatch|手动触发|触发/i })).toBeVisible()
    }
  })
