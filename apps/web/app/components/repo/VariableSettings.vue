...
import { Eye, EyeOff, FileText, Key, Plus, Trash2 } from 'lucide-vue-next'
...
import DotenvImportDialog from './DotenvImportDialog.vue'
import type { ImportSummary } from '@/composables/useDotenvImport'
...
const importDialogKind = ref<VariableKind>('plain')
const importFlash = ref<{ kind: 'default' | 'destructive'; text: string } | null>(null)
let importFlashTimer: ReturnType<typeof setTimeout> | null = null

function openImportDialog(kind: VariableKind) {
  importDialogKind.value = kind
...
function onImported(summary: ImportSummary) {
  ...
  const overwritten = summary.overwritten.length
  const failed = summary.failed.length

  if (failed > 0 && created === 0 && overwritten === 0) {
    // All failed / network error
    const detail = summary.failed.map(f => `${f.key}: ${f.error}`).join(', ')
    showImportFlash('destructive', t('repo.variables.import.flashAllFailed', { detail }))
  } else if (failed > 0) {
    // Partial failure
    const parts: string[] = []
    if (created > 0) parts.push(t('repo.variables.import.flashCreated', { n: created }))
    if (overwritten > 0) parts.push(t('repo.variables.import.flashOverwritten', { n: overwritten }))
    showImportFlash(
      'default',
      t('repo.variables.import.flashPartial', { summary: parts.join(', '), n: failed }),
    )
  } else {
    // All success
    const parts: string[] = []
    if (created > 0) parts.push(t('repo.variables.import.flashCreated', { n: created }))
    if (overwritten > 0) parts.push(t('repo.variables.import.flashOverwritten', { n: overwritten }))
    showImportFlash('default', t('repo.variables.import.flashSuccess', { summary: parts.join(', ') }))
  }
}

function showImportFlash(kind: 'default' | 'destructive', text: string) {
  if (importFlashTimer) clearTimeout(importFlashTimer)
  importFlash.value = { kind, text }
  importFlashTimer = setTimeout(() => {
    importFlash.value = null
  }, 8000)
}
...
<template>
  <div class="space-y-6">
    ...
    <!-- Import flash -->
    <Alert v-if="importFlash" :variant="importFlash.kind">
      <AlertDescription>{{ importFlash.text }}</AlertDescription>
    </Alert>
    ...
    <Card>
      <CardHeader class="flex flex-row items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-2">
          <Button variant="outline" size="sm" @click="openImportDialog('plain')">
            <FileText class="size-4" />
            {{ t('repo.variables.import.button') }}
          </Button>
    ...
    <Card>
      <CardHeader class="flex flex-row items-center justify-between gap-2">
        ...
        <Button variant="outline" size="sm" @click="openImportDialog('secret')">
          <FileText class="size-4" />
          {{ t('repo.variables.import.button') }}
        </Button>
      </CardHeader>
    ...
    <!-- Dotenv import dialog -->
    <DotenvImportDialog
      v-model:open="importDialogOpen"
      :owner="owner"
      :name="name"
      :initial-kind="importDialogKind"
      :existing-variable-names="variables.map(v => v.name)"
      :existing-secret-names="secrets.map(s => s.name)"
      @imported="onImported"
    />
  </div>
</template>
