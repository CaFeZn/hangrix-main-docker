...
// Whether the submit button should be disabled because a "其他…"
// option is selected but the text field is empty.
const otherTextMissing = computed(() => {
  for (const q of props.questionnaire.questions) {
    if (q.type === 'text_input') continue
    const entry = draft[q.id]
    if (!entry?.option_ids?.length) continue
    if (entry.option_ids.includes(OTHER_OPTION_ID) && !entry.text?.trim()) {
      return true
    }
  }
  return false
...
function optionLabelWithOtherText(qid: number, oid: string): string {
  const label = optionLabel(qid, oid)
  if (oid !== OTHER_OPTION_ID) return label
  const entry = mySubmission.value?.answers.find((a) => a.question_id === qid)
  if (entry?.text) return `${label} (${entry.text})`
  return label
}
</script>
      ...
      <div class="space-y-3 px-4 py-3 text-sm">
        ...
        <template v-if="isOpen && !hasSubmitted">
          ...
          <div class="space-y-2">
            <Button
              v-if="isLoggedIn"
              :disabled="submitting || locked || otherTextMissing"
              @click="handleSubmit"
            >
                ...
                >
                  {{
                    mySubmission
                      ...
                      .option_ids!.map((oid) => optionLabelWithOtherText(q.id, oid))
                      .join(', ')
                  }}
                </template>
                <template v-else>
