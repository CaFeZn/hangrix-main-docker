...
async function loadDetail(id: number, opts?: { silent?: boolean }) {
  if (!opts?.silent) {
  ...
  try {
    ...
    if (opts?.silent) detailError.value = null
  } catch (e: any) {
    if (!opts?.silent) {
...
function startPoll() {
  ...
  timer = setInterval(() => {
    ...
    loadList()
    if (selectedId.value && !detailInFlight) {
      detailInFlight = true
      loadDetail(selectedId.value, { silent: true }).finally(() => {
...
const pendingReviewers = computed<string[]>(() => {
  const r = detail.value?.review
  if (!r) return []
  if (!r.required_reviewers?.length) return []
  return r.pending_reviewers ?? []
})
</script>
      ...
      <template v-else>
        ...
        <Tabs v-model="detailTab">
          ...
          <TabsContent value="reviews" class="mt-3">
            <Card class="gap-0 py-0">
              <CardContent class="space-y-3 p-4">
                ...
                <!-- Pending reviewers -->
                <div v-if="pendingReviewers.length > 0" class="space-y-1.5">
                  <p class="flex items-center gap-1 text-xs text-muted-foreground">
                    <Clock class="size-3" />
                    {{ t('issue.review.pending.title', { n: pendingReviewers.length }) }}
                  </p>
                  <ul class="space-y-1.5">
                    <li
                      v-for="r in pendingReviewers"
                      :key="r"
                    ...
                    >
                      ...
                      <Badge :class="verdictClass('pending')" variant="secondary" class="shrink-0">
                        {{ t('issue.review.pending.badge') }}
                      </Badge>
                    </li>
                  </ul>
                </div>
          ...
          <TabsContent value="checks" class="mt-3">
            <CheckRunPanel
              v-if="detail"
              :key="detail.contribution.id"
              :owner="owner"
              :name="name"
              :issue-number="issueNumber"
              :commit-sha="detail.contribution.head_sha"
            />
          </TabsContent>
        </Tabs>
      </template>
