...
type CreateRequest struct {
	...
	MaxOutputTokens int
	// ReasoningEffort is forwarded on the wire as `reasoning.effort`
	// (typically "low" / "medium" / "high"). Empty means the field is
	// omitted from the request body — the upstream applies its model
	// default. The proxy and each upstream adapter decide what to do
	// with it; for OpenAI Response API it lands on the o-series thinking
	// budget knob, for Anthropic it maps onto extended thinking, for
	// openai-compat it's a hint some vendors honour and others ignore.
	ReasoningEffort string

	// Thinking is the Anthropic thinking-mode toggle ("adaptive" /
	// "enabled" / "disabled"). Forwarded on the wire as the top-level
	// `thinking` string; only the Anthropic upstream adapter consumes
	// it. Empty means the field is omitted from the request body — the
	// upstream applies its model default (no extended thinking on
	// Claude 4.7+).
	Thinking string
}
...
//
// Status / IncompleteReason mirror the Response API top-level `status`
// and `incomplete_details.reason` fields. They are diagnostic only —
// the loop primarily branches on (Content + ToolCalls) being empty,
// because some upstreams (OpenAI Response API gateways, third-party
// proxies) ship a 2xx body with `status:"completed"` but `output:null`
// or `output:[]` even when usage.output_tokens > usage.reasoning_tokens
// — i.e. the model produced non-reasoning content that the upstream
// dropped before serialising. Surfacing these fields lets us log WHY
// the response was empty even when the upstream's status claim is
// misleading.
...
type Usage struct {
	...
	OutputTokens    int
	TotalTokens     int
	ReasoningTokens int // subset of OutputTokens spent on hidden reasoning
}
...
func buildRequestBody(req *CreateRequest) ([]byte, error) {
	...
	if req.ReasoningEffort != "" {
		// Response API shape: `reasoning: {effort: "..."}`. The proxy's
		// ParseResponsesAPIRequest reads it back into Request.ReasoningEffort
		// and each adapter forwards it through its own wire (OpenAI keeps
		// the same shape, Anthropic remaps to thinking budget, openai-compat
		// passes a flat `reasoning_effort` string).
		wire["reasoning"] = map[string]any{"effort": req.ReasoningEffort}
	}
	if req.Thinking != "" {
		// Hangrix-private knob on top of the OpenAI Responses-API shape:
		// the proxy parses it back into Request.Thinking and the Anthropic
		// adapter turns it into `thinking: {type: ...}`. Non-Anthropic
		// adapters ignore it.
...
func parseResponse(resp *http.Response) (*CreateResponse, error) {
	...
	out := &CreateResponse{
		ID:               wire.ID,
		Status:           wire.Status,
		IncompleteReason: wire.IncompleteDetails.Reason,
