...
// TestSleepBufferedNotificationStillCountsAsRunningJob closes the wake-up
// race fixed in issue #372. Once the timer has fired but before the runtime
// drains the buffered completion notification, HasRunningJobs must still
// report outstanding work; otherwise the agent transport can EOF the wake
// before the notification is processed.
func TestSleepBufferedNotificationStillCountsAsRunningJob(t *testing.T) {
	t.Parallel()
	bundle := local.Build()
	tools := byName(bundle.Tools)
	sleep := tools["sleep"]
	...
	_, err := sleep.Call(context.Background(), mustJSON(map[string]any{"seconds": 1, "reason": "buffered-notification-race"}))
	if err != nil {
		t.Fatalf("sleep(seconds=1): %v", err)
	}

	// Wait long enough for the timer to fire and enqueue its notification,
	// but do NOT drain NotificationCh yet.
	time.Sleep(1200 * time.Millisecond)

	if n := bundle.Async.HasRunningJobs(); n != 1 {
		t.Fatalf("HasRunningJobs after timer fire but before notification drain = %d, want 1", n)
	}

	select {
	case msg := <-bundle.Async.NotificationCh():
		if !strings.Contains(msg, `notification.sleep.finished`) {
			t.Fatalf("buffered notification = %q, want sleep completion event", msg)
		}
	default:
		t.Fatal("expected sleep notification to already be buffered")
	}

	deadline := time.Now().Add(500 * time.Millisecond)
	for time.Now().Before(deadline) {
		if bundle.Async.HasRunningJobs() == 0 {
			return
		}
		time.Sleep(10 * time.Millisecond)
	}
	t.Fatalf("HasRunningJobs after draining notification = %d, want 0", bundle.Async.HasRunningJobs())
}
...
// ----- edit substring matching tests ----------------------------------------

// TestEditFindMidLineAllowed verifies that find can match mid-line substrings.
func TestEditFindMidLineAllowed(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "f.txt")
	...
	if err != nil {
		t.Fatalf("edit with mid-line find: %v", err)
	}
...
// TestEditAnchorMidLineAllowed verifies that anchor can be a mid-line
// substring and still narrow the search region.
func TestEditAnchorMidLineAllowed(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "f.txt")
	...
	if err != nil {
		t.Fatalf("edit with mid-line anchor: %v", err)
	}
...
// TestEditCRLFSubstringFind verifies that substring find works on CRLF files
// after normalisation and original CRLF style is preserved.
func TestEditCRLFSubstringFind(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "crlf.txt")
	...
	if err != nil {
		t.Fatalf("edit with mid-line find on CRLF file: %v", err)
	}
	...
	if !bytes.Contains(body, []byte("\r\n")) {
		t.Fatal("expected CRLF endings to be preserved")
	}
}
...
// ----- edit anchor + all=true tests -----------------------------------------

// TestEditAnchorSelectsClosestOccurrence verifies that when multiple matches
// exist in the anchor window, edit picks the one closest to the anchor line.
func TestEditAnchorSelectsClosestOccurrence(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "f.txt")
	...
	body, _ := os.ReadFile(path)
	if got := string(body); got != "target\nanchor here\nchanged\n" {
		t.Errorf("unexpected content: %q", got)
	}
}

// TestEditReplaceAllIncludesMidLineOccurrences verifies all=true replaces
// every occurrence, including mid-line matches.
func TestEditReplaceAllIncludesMidLineOccurrences(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	path := filepath.Join(dir, "f.txt")
	...
	if err != nil {
		t.Fatalf("edit all=true: %v", err)
	}

	body, _ := os.ReadFile(path)
	if got := string(body); got != "hello earth\nearth\n" {
		t.Errorf("expected 'hello earth\\nearth\\n', got: %q", got)
	}
}
