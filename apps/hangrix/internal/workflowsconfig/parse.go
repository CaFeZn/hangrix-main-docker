...
func decodeOn(node *yaml.Node) ([]EventTrigger, []string) {
	...
	for i := 0; i < len(node.Content); i += 2 {
		...
		case EventContributionPush:
			// v1 takes no filters; empty object is legal.
...
func decodeJobs(node *yaml.Node) ([]JobDefinition, []string) {
	...
	for i := 0; i < len(node.Content); i += 2 {
		...
		for si, sw := range jw.Steps {
			...
			// Reject step IDs that use the reserved "phase:" prefix.
			if stepID := sw.Id; stepID != "" && strings.HasPrefix(stepID, "phase:") {
				errs = append(errs, fmt.Sprintf("%s.id: %q uses reserved prefix \"phase:\"", sp, stepID))
			}
