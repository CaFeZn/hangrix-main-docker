...
const (
	...
	// EventAgentWake is the trigger event for the hidden internal `_agent`
	// workflow that the agent-session spawner creates on every wake. Not
	// listed in validEventNames because user-authored .hangrix/workflows
	// yaml is forbidden to subscribe to it (the trigger only fires from
	// inside the platform).
