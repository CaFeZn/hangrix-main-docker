...
type HostConfig struct {
	...
	// Silence is the optional repository silence configuration.
	// nil means no silence block was declared in agents.yml —
	// the SilenceGate has no schedules and silence is purely
	// manual (API-driven). An empty Schedules slice (Silence != nil)
	// means the block exists but has no schedules, also purely manual.
...
// explicitly set, override team" (non-nil). Field-level merge happens at
// session spawn time in modules/agent_session/service.resolveLLM.
//
// Sampling parameters (`temperature`, `top_p`, `top_k`) are intentionally
// not exposed: Claude Opus 4.7+ rejects non-default values with 400, and
// extended-thinking on older Claudes also drops them. Steer behaviour via
// the system prompt and `reasoning_effort`/`thinking` instead.
type LLMConfig struct {
	// Model is a name string that must match a defined LLM model/group at
	// runtime (see the llm_provider routing). Empty at role level = inherit
	// team's model; empty at team level is rejected by the parser.
	Model string
	...
	// MaxContextTokens is deprecated. The field has been removed;
	// agents now query the model spec via GET /api/v1/models/{name}
	// at startup to determine context window.
	...
	// / "max" drive the Anthropic adapter's `output_config.effort` (on
	// Claude 4.6+) or `thinking.budget_tokens` translation (legacy);
	...
	// Thinking selects the Anthropic extended-thinking mode: one of
	// "adaptive" (Claude 4.6+ canonical — `thinking: {type: adaptive}`),
	// "enabled" (legacy `thinking: {type: enabled, budget_tokens: N}`,
	// budget derived from ReasoningEffort), or "disabled" (omit the
	// `thinking` field). nil = inherit (or "omit on the wire" at the
	// bottom of the chain). Adapters other than Anthropic ignore it.
	// Pointer-to-empty-string explicitly resets a team default back to
	// "don't send a thinking parameter".
	Thinking *string
}
