...
var (
	...
	// ErrInvalidLLMParam fires for negative max_output_tokens /
	// max_context_tokens or an unrecognised `thinking` value.
	...
	// ErrInvalidSilence fires when the `silence:` block in agents.yml
	// is malformed: empty/duplicate schedule name, invalid cron/duration/
	// timezone, or any other schema violation.
	ErrInvalidSilence = errors.New("invalid silence config")
)
