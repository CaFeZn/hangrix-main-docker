...
// OtherOptionID is the reserved option ID for the platform-injected
// "其他…" virtual option attached to every choice question at
// render time. It is never persisted to the options JSONB.
const OtherOptionID = "__other__"

// OtherOptionLabel is the default zh-CN label rendered for the
// virtual "其他…" option. (i18n is out of scope per the spec.)
...
// MaxOptionLabelLen is the maximum allowed length for a choice option's label field.
const MaxOptionLabelLen = 1000
...
type QuestionResult struct {
	...
	OtherResponses []TextResponse `json:"other_responses,omitempty"`
...
func (p *CreateParams) Validate() []FieldError {
	...
	for i, q := range p.Questions {
		...
		if !q.Type.Valid() {
			errs = append(errs, FieldError{Field: prefix + ".type", Code: "invalid", Message: "Question type must be one of: single_choice, multi_choice, or text_input"})
		}
		...
		case QtypeSingleChoice, QtypeMultiChoice:
			...
			for j, o := range q.Options {
				if o.ID == OtherOptionID {
					errs = append(errs, FieldError{Field: prefix + ".options[" + strconv.Itoa(j) + "].id", Code: "reserved", Message: "option ID '__other__' is reserved and must not be used"})
				}
				if len(strings.TrimSpace(o.Label)) == 0 {
					errs = append(errs, FieldError{Field: prefix + ".options[" + strconv.Itoa(j) + "].label", Code: "missing"})
				} else if len(o.Label) > MaxOptionLabelLen {
					errs = append(errs, FieldError{Field: prefix + ".options[" + strconv.Itoa(j) + "].label", Code: "too_long", Message: "max 1000 characters"})
				}
...
func ValidateAnswer(questions []Question, perQ map[int64]AnswerValue) []FieldError {
	...
	for qID, av := range perQ {
		...
		hasOther := containsOther(av.OptionIDs)
		...
		case QtypeSingleChoice:
			if len(av.OptionIDs) != 1 {
				...
				if !optMap[qID][av.OptionIDs[0]] && av.OptionIDs[0] != OtherOptionID {
			...
			if hasOther {
				if strings.TrimSpace(av.Text) == "" {
					errs = append(errs, FieldError{Field: "answers", Code: "other_text_required", Message: "text is required when '其他…' is selected for question " + strconv.FormatInt(qID, 10)})
				}
			} else if av.Text != "" {
		...
		case QtypeMultiChoice:
			...
			for _, oid := range av.OptionIDs {
				if !optMap[qID][oid] && oid != OtherOptionID {
...
// QuestionWithOtherOption returns a copy of q with the platform-injected
// "其他…" virtual option appended when q is single_choice or multi_choice.
// Text-input questions are returned unchanged. This is a pure function and
// never mutates the original.
func QuestionWithOtherOption(q Question) Question {
	if q.Type != QtypeSingleChoice && q.Type != QtypeMultiChoice {
		return q
	}
	out := q
	out.Options = make([]Option, len(q.Options)+1)
	copy(out.Options, q.Options)
	out.Options[len(q.Options)] = Option{ID: OtherOptionID, Label: OtherOptionLabel}
	return out
}

// containsOther reports whether optionIDs includes the reserved OtherOptionID.
func containsOther(optionIDs []string) bool {
