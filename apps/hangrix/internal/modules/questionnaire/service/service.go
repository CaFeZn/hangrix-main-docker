...
func (s *Service) BuildResult(ctx context.Context, qID int64) (*domain.Result, error) {
	...
	for _, q := range qn.Questions {
		...
		case domain.QtypeSingleChoice, domain.QtypeMultiChoice:
			...
			qr.Tallies = tallies
			qr.OtherResponses = buildOtherResponses(q, answers)
...
func buildChoiceTallies(q domain.Question, answers []*domain.Answer) []domain.ChoiceTally {
	// Use the virtual options that include the "其他…" entry.
	vq := domain.QuestionWithOtherOption(q)
	...
	for _, o := range vq.Options {
		...
		tallies = append(tallies, domain.ChoiceTally{
			...
			Count:    c,
			Percent:  pct,
			IsOther:  o.ID == domain.OtherOptionID,
		})
	}
	return tallies
}
...
// buildOtherResponses collects free-text responses from users who selected
// the "其他…" (OtherOptionID) virtual option for a choice question.
func buildOtherResponses(q domain.Question, answers []*domain.Answer) []domain.TextResponse {
	var responses []domain.TextResponse
	for _, a := range answers {
		av, ok := a.PerQuestion[q.ID]
		if !ok {
