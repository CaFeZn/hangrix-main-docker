...
// resolveCheckers fetches CI check items for a commit SHA. Returns an empty
// slice (not nil) when there are no checks or the CheckReader is unavailable.
func (r *Registry) resolveCheckers(ctx context.Context, repoID int64, headSHA string) []apiCheckItem {
	if r.deps.CheckReader == nil || headSHA == "" {
		return []apiCheckItem{}
	}
	items, err := r.deps.CheckReader.ListChecksByCommit(ctx, repoID, headSHA)
	if err != nil {
		return []apiCheckItem{}
	}
	if items == nil {
	...
	out := make([]apiCheckItem, len(items))
...
func (r *Registry) fanCommentMentionsForIssue(ctx context.Context, sess *runnerdomain.AgentSession, repoID int64, targetIssueNumber, sourceIssueNumber int32, c *issuedomain.Comment) {
	...
	// Resolve the triggering actor: use the session's agent role as the
	// primary actor identity (new path), with the legacy CreatedBy
	// user ID as a fallback. Mirrors CreateIssue's actor resolution.
	triggerActor := resolvePlatformCommentActor(r.deps.ActorResolver, sess)
...
// resolvePlatformCommentActor resolves the triggering actor for
// agent-authored comments. Uses the session's role key to construct
// an agent actor ref; returns nil when the actor resolver is not
// wired (tests). Nil-safe — the spawner falls back to ActorID.
func resolvePlatformCommentActor(resolver actordomain.Resolver, sess *runnerdomain.AgentSession) *actor.Ref {
	if resolver == nil || sess == nil || sess.RoleKey == "" {
		return nil
	}
	resolved, err := resolver.From(context.Background(), actor.AgentRef(sess.RoleKey))
	if err != nil {
		return nil
	}
	return resolved
