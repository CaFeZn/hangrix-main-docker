...
type apiIssueResponse struct {
	...
	Checkers     []apiCheckItem `json:"checkers"`
}
...
func (s *APIService) ReadIssue(ctx context.Context, p *apidomain.Actor) (any, error) {
	...
	return &apiIssueResponse{
		...
		Checkers:     s.r.resolveCheckers(ctx, scope.repo.ID, scope.issue.HeadSHA),
	}, nil
}
...
func (s *APIService) ReadIssueByNumber(ctx context.Context, p *apidomain.Actor, issueNumber int64) (any, error) {
	...
	return &apiIssueResponse{
		...
		Todos:        todosToAPIDTO(todos),
		TodoSummary:  todoSummaryToAPIDTO(todoSummary),
		Checkers:     s.r.resolveCheckers(ctx, scope.repo.ID, iss.HeadSHA),
	}, nil
}
...
func (s *APIService) CreateIssue(ctx context.Context, p *apidomain.Actor, title, body string, parent bool) (any, error) {
	...
	if s.r.deps.Spawner != nil {
		var payload []byte
		if b, err := json.Marshal(map[string]any{"issue_title": iss.Title, "issue_body": iss.Body}); err == nil {
			payload = b
		}
...
func (s *APIService) CreateComment(ctx context.Context, p *apidomain.Actor, body, filePath string, line int) (any, error) {
	...
	c, err := s.r.deps.Issues.CreateAgentComment(ctx, scope.issue.ID, p.RoleKey, body, strings.TrimSpace(filePath), line, 0)
	if err != nil {
...
func (s *APIService) CreateCrossIssueComment(ctx context.Context, p *apidomain.Actor, targetIssueNumber int64, body, filePath string, line int) (any, error) {
	...
	// Prepend a source-issue attribution so readers on the target issue
	// know where the comment originated. Uses a markdown blockquote for
	// clear visual separation from the comment body.
	attributedBody := fmt.Sprintf("> This comment was cross-posted from issue #%d\n\n%s", *p.IssueNumber, body)

	c, err := s.r.deps.Issues.CreateAgentComment(ctx, target.ID, p.RoleKey, attributedBody, strings.TrimSpace(filePath), line, int64(*p.IssueNumber))
	if err != nil {
		return nil, fmt.Errorf("create comment: %w", err)
	}
...
// GetComment reads a single comment by id. Only comments on the current
// session's issue or its direct parent are accessible; lookups for any other
// issue return an appropriate "not found" error without leaking information.
func (s *APIService) GetComment(ctx context.Context, p *apidomain.Actor, commentID int64) (any, error) {
	...
	// Case A (comment on current issue) — existing happy path.
	if c.IssueID == scope.issue.ID {
		return commentToDTO(c), nil
	}

	// Case B (comment on direct parent) — new happy path.
	if scope.issue.ParentID != 0 && c.IssueID == scope.issue.ParentID {
		return commentToDTO(c), nil
	}

	// Case C: current issue has no parent.
	if scope.issue.ParentID == 0 {
		return nil, fmt.Errorf("comment %d not found. This issue has no parent, so only comments on the current issue are accessible.", commentID)
	}

	// Case D: comment belongs to an unrelated issue (grandparent, sibling,
	// child, or any other issue outside the current↔parent boundary).
	return nil, fmt.Errorf("comment %d not found. Only comments on the current issue or its direct parent are accessible.", commentID)
}

// commentToDTO serialises a domain Comment into the apiComment DTO used in
// all API responses.
func commentToDTO(c *issuedomain.Comment) *apiComment {
	author := c.AuthorName
	if c.AgentRole != "" {
		author = c.AgentRole
	}
...
func (s *APIService) CreateTodo(ctx context.Context, p *apidomain.Actor, content, status string, position int) (any, error) {
	...
	if !st.Valid() {
		return nil, errors.New("Status must be one of: todo, in_progress, or done")
	}
...
func (s *APIService) ReadContribution(ctx context.Context, p *apidomain.Actor, id int64) (any, error) {
	...
	contrib := contributionSummaryToAPI(c)
	contrib.Checkers = s.r.resolveCheckers(ctx, scope.repo.ID, c.HeadSHA)
	return map[string]any{
...
func (s *APIService) CreateReview(ctx context.Context, p *apidomain.Actor, contributionID int64, value, reason string) (any, error) {
	...
	if !v.Valid() {
		return nil, errors.New("Value must be one of: approve, reject, or abstain")
	}
	if contributionID <= 0 {
	...
	// Fire review_vote.posted so roles subscribed to it (e.g.
	// maintainer) wake on the newly-cast vote. Mirrors the pattern
	// used by issue.opened / issue.comment / commit.pushed.
	if s.r.deps.Spawner != nil {
		triggerActor := s.resolvePlatformActor(ctx, p)
		firePayload, _ := json.Marshal(map[string]any{
			"event_id":        evt.ID,
			"contribution_id": c.ID,
			"value":           string(v),
			"reason":          reason,
			"reviewed_sha":    c.HeadSHA,
		})
		if _, err := s.r.deps.Spawner.OnTrigger(ctx, agentsessiondomain.TriggerInput{
			...
			CauseID:     strconv.FormatInt(evt.ID, 10),
			RepoID:      scope.repo.ID,
			IssueNumber: int32(scope.issue.Number),
			Actor:       triggerActor,
			Payload:     firePayload,
		}); err != nil {
			log.Printf("platform_api: fire review_vote.posted repo=%d issue=%d contrib=%d: %v",
				scope.repo.ID, scope.issue.Number, c.ID, err)
		}
	}
...
func toAPIQuestionnaire(qn *questionnairedomain.Questionnaire) map[string]any {
	...
	for _, q := range qn.Questions {
		vq := questionnairedomain.QuestionWithOtherOption(q)
		qi := map[string]any{
			"id":       vq.ID,
...
func toAPIResult(r *questionnairedomain.Result) map[string]any {
	...
	for qid, qr := range r.ByQuestion {
		...
		if qr.OtherResponses != nil {
			var otherResponses []map[string]any
			for _, tr := range qr.OtherResponses {
				otherResponses = append(otherResponses, map[string]any{
					"user_id":      tr.UserID,
			...
			qi["other_responses"] = otherResponses
		}
		if qr.Responses != nil {
...
// GetModel returns the model spec for the given model name.
func (s *APIService) GetModel(ctx context.Context, p *apidomain.Actor, name string) (any, error) {
	if s.r.deps.LLMLookup == nil {
		return nil, errors.New("llm lookup not available")
	}
	spec, err := s.r.deps.LLMLookup.GetModelSpec(ctx, name)
	if err != nil {
		if errors.Is(err, llmproviderdomain.ErrModelNotFound) {
			return nil, apidomain.ErrModelNotFound
		}
		return nil, err
	}
	return map[string]any{
		...
		"reasoning_effort_map": spec.ReasoningEffortMap,
	}, nil
}
