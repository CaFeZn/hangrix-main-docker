...
const (
	...
	// CauseKindSubIssueMerged — a child issue was merged,
	// firing sub_issue.merged on the parent.
	CauseKindSubIssueMerged CauseKind = "sub_issue_merged"

	// CauseKindSubIssueClosed — a child issue was closed,
	// firing sub_issue.closed on the parent.
	CauseKindSubIssueClosed CauseKind = "sub_issue_closed"
)
...
const (
	...
	// SpawnActionSuppressed — the spawn was blocked by a cross-cutting
	// gate (repo silence, etc.). No row was created or modified; the
	// action field carries a reason for audit/UI consumption.
	SpawnActionSuppressed SpawnAction = "suppressed"
)
...
type Controller interface {
	...
	// Reset archives the session row (preserving it as audit history)
	// and spawns a brand-new session row for the same (repo, issue,
	// role) tuple. The new row starts in pending, with a freshly
	// minted session token, no inherited messages or inputs, and no
	// container — the next trigger or a user-initiated Resume picks
	// it up.
	//
	// Refuses archived rows (ErrNotResettable: already gone) and
	// pending / claimed rows (ErrSessionLive: nothing to reset yet).
	// Running rows are accepted — Reset internally shuts down the
	// live container before archiving, so the session exits cleanly.
	//
	// Uses final-consistency: archive may succeed but spawn may fail.
	// A failed spawn leaves the user without a new session, but the
	// next trigger on the issue will naturally spawn a fresh row
	// because the most-recent row is now archived.
	Reset(ctx context.Context, sessionID int64) (newSessionID int64, err error)
}
...
// ErrNotResettable is returned by Controller.Reset when a session row
// cannot be reset (archived — the issue is closed and the slot is dead).
var ErrNotResettable = errors.New("session not resettable")
