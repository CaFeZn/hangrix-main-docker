...
func (h *Handler) SyncIssueBranch(ctx context.Context, repo *repodomain.Repo, fsPath string, iss *domain.Issue, actorID int64, agentRole string) error {
	...
	if len(newCommits) > 0 {
		...
		// Dispatch issue.push workflows. Best-effort — a workflow dispatch
		// failure must never block the head-sha update.
		h.dispatchIssuePush(ctx, repo, iss, headSHA, oldRef, changedPaths)
	}
	return nil
}
...
// dispatchIssuePush fans an issue.push workflow trigger.
func (h *Handler) dispatchIssuePush(ctx context.Context, repo *repodomain.Repo, iss *domain.Issue, headSHA, oldRef string, changedPaths []string) {
	if h.workflowDispatcher == nil {
		return
	}
	h.workflowDispatcher.DispatchIssuePush(ctx, workflowdomain.Ref{
		ID:            repo.ID,
		Name:          repo.Name,
		DefaultBranch: repo.DefaultBranch,
		OwnerName:     repo.OwnerName,
	}, workflowdomain.IssuePushInput{
		IssueNumber:  iss.Number,
		BranchName:   "refs/heads/" + iss.BranchName,
		CommitSHA:    headSHA,
		OldCommitSHA: oldRef,
		ChangedPaths: changedPaths,
		Cause:        "contribution_apply",
	})
}